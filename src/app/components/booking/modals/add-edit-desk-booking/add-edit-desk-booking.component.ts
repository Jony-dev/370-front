...
export class AddEditDeskBookingComponent implements OnInit {
  ...
  getBuildingTables(){
    ...
    this.api.getBuildingTables(buildingId,this.booking.dateId).subscribe(x => 
    {
      this.tables = x;
      if(this.editing)
        this.tables.push({id: this.booking.tableId, name : this.booking.tableName, ttypeId : null, floorId : null, barcode : null, buildingId : null, buildingName : null, floorName : null});
    });
